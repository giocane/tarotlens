...
| `Produits` | `id, cat, name, name_en, tag, tag_en, cards, format, format_en, weight, weight_en, delivery, delivery_en, price, badge, glyph, grad, desc, desc_en, images, inStock, hero, preorder, extension, extensionProductId, extensionPrice, extensionDesc, extensionDesc_en, hidden` | Ligne d'en-tête exacte définie par la constante `PRODUITS_ENTETES` dans `Code.gs` (source de vérité — si les deux divergent, c'est le Sheet qui pilote). Colonne `images` = URLs séparées par `\|`, la première sert de visuel de couverture. Colonne `preorder` = booléen, bascule le texte du bouton de la bannière hero du produit sur « Précommandez votre tarot » (texte éditable dans l'onglet Textes de `admin.html`). Colonnes `extension*` = pack optionnel « deck + extension » proposé sur la fiche produit : `extension` (booléen) active le bloc, `extensionProductId` référence l'`id` du produit extension dans ce même onglet, `extensionPrice` est le prix du pack, `extensionDesc`/`extensionDesc_en` son descriptif. Le stock des deux produits (deck et extension) est décompté indépendamment à la commande. Colonne `hidden` = booléen, masque le produit des grilles du site (utile pour un produit qui ne doit être accessible que comme `extensionProductId` d'un autre, jamais vendu seul). Catalogue complet, éditable depuis `admin.html` (ou à la main). |
